...
### 1.1 Changes from v6.2

| Area                               | Prior                                                                                                                  | Current                                                                                                                                                 | Rationale                                                                                                                                                      |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Explicit document profiling        | System-definition intent was inferred indirectly from system_metadata                                                  | document_profile explicitly distinguishes project_instance, project_instance_express, and system_definition roots                                       | Makes authored document intent machine-explicit and allows the schema to require the authoritative top-level surface only for system-definition artifacts.     |
| Topology closure                   | active_tiers enforced membership loosely and left topology consequences to downstream logic                            | active_tiers are restricted to canonical order variants and system-definition topology obligations are stated as first-class invariants                 | Closes mandatory-tier presence, ordering, and representative-coverage ambiguity without introducing a second competing topology model.                         |
| Lifecycle authority simplification | Lifecycle behavior was split across status_transitions and prohibited_transitions, with composite raw operation tokens | status_transitions are the sole lifecycle authority and transition metadata decomposes canonical operation, phase, side-effect, and prerequisites       | Removes dual-authority drift and keeps operation identity machine-normalized.                                                                                  |
| ARE contract hardening             | ARE activation states, E5 scoring_profile requirements, and custom-profile structure were under-typed                  | ARE activation states are structurally typed, E5 requires scoring_profile, custom profile structure is machine-shaped, and score bounds are constrained | Strengthens the schema front door while reserving higher-order profile-resolution and score-band ordering checks for deterministic ARE conformance validation. |
| Operation namespace normalization  | Operations, lifecycle rows, and scaffold comments mixed canonical names with phase and alias tokens                    | The canonical operation surface is closed and UNBUNDLE_EXECUTE is the sole commit-phase token                                                           | Keeps validators, logs, and API-like surfaces aligned to one operation vocabulary.                                                                             |
| Rule identifier typing             | Invariant, atomic-rule, and extension-rule identifiers remained partially free-form                                    | Rule-ID families are centralized and typed consistently across the schema                                                                               | Reduces drift risk and makes malformed rule references fail earlier.                                                                                           |
| Express mode closure               | Express Mode groups and top-level express authority remained under-enforced                                            | Group compositions are fixed structurally and express-capable document profiles require the full express_mode authority block                           | Prevents authored Express Mode files from silently redefining group structure or omitting their governing UNBUNDLE_SCAN / UNBUNDLE_EXECUTE contract.           |
...
| INV-3     | active_tiers must be one of four canonical ordered sets only: [SIL, GPCL, FCL, SAL, ICL, CDL, ISL], [XPD, SIL, GPCL, FCL, SAL, ICL, CDL, ISL], [SIL, GPCL, FCL, CL, SAL, ICL, CDL, ISL], or [XPD, SIL, GPCL, FCL, CL, SAL, ICL, CDL, ISL]. Every node tier must belong to active_tiers, and every system_definition artifact must contain at least one representative node for each active tier. |
...
| ACTIVE            | DIRTY             | MODIFY    | -                      | Direct MODIFY of an ACTIVE node enters DIRTY pending re-validation.                                                                                                                          |
| ACTIVE            | DIRTY             | MODIFY    | -                      | Covers DIRTY propagation side-effects on descendants after ancestor-side mutations.                                                                                                          |
...
| SUPERSEDE_PENDING | SUPERSEDED        | SUPERSEDE | gc-008                 | All SUPERSEDE steps completed successfully. prior_status cleared.                                                                                                                            |
| SUPERSEDE_PENDING | -                 | SUPERSEDE | gc-009                 | Source node reverts to its recorded prior_status value. prior_status field cleared. No replacement node created. No parent_ids modified. SUPERSEDE_FAILED logged to reconciliation manifest. |
...
| ----------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| Express (4 Groups)      | Adjacent tiers bundled into groups; expandable through the UNBUNDLE_SCAN / UNBUNDLE_EXECUTE protocol. | Small-to-medium projects.                  |
| Full (All Active Tiers) | Every active tier independently specified.                                                            | Complex, regulated, or enterprise systems. |

Express Mode is not a reduced system — it is Full Mode with grouped presentation. The UNBUNDLE_SCAN / UNBUNDLE_EXECUTE protocol expands any group into its constituent tiers without information loss or invention. When UNBUNDLE_EXECUTE succeeds, parent_ids automatically wire to the immediately superior unbundled tier, satisfying CIT-R2 without manual intervention.
...
| ARE-R2 | Each candidate carries ARE::confidence_score (0.0–1.0) computed according to the scoring_profile declared in the ARE Extension contract. The declared profile must be a standard profile (standard_v1 or conservative_v1) or a custom profile with all required_fields declared per are_scoring_profiles.custom.required_fields. Deterministic ARE conformance validation resolves the scoring_profile reference, confirms score-band ordering and non-overlap, and enforces reproducible scoring: identical source evidence inputs must produce identical scores under the same profile (AX-3).                                                                                                     |
...
Custom profiles that omit any required_fields entry fail extension contract validation under EXT-R1. Deterministic ARE conformance validation is additionally responsible for profile-reference resolution, score-band ordering, and score-band non-overlap checks.
...
- [ ] System-definition artifacts include at least one representative node for every tier listed in active_tiers.
...
- [ ] ARE scoring_profile is declared in the E5 Extension contract and references a valid entry in are_scoring_profiles. Deterministic ARE conformance validation resolves that reference before any candidate promotion or review surface is considered valid.
- [ ] Custom ARE profiles satisfy all required_fields and conform to are_scoring_profiles.custom.profile_template; all score_bands are ordered, non-overlapping, and bounded within [0.0, 1.0].
- [ ] Candidates promoted below minimum_surfacing_threshold carry override_flag: true with a non-empty human_rationale in pending_items.
...
| Express Mode       | A four-group consumption mode; groups are unbundleable to Full Mode tiers through the UNBUNDLE_SCAN / UNBUNDLE_EXECUTE protocol.                                                                                                                                                                                                                                     |
...
| 5.0     | 2026-03-25 | Issue-driven refinement: 13 v4.0 issues resolved; SUPERSEDE atomicity with SUPERSEDE_PENDING transient state and prior_status rollback; verification_mode on atomic rules; FCL-R7 data entity enumeration; ARE tri-state lifecycle with checkpoint persistence; DDE confirmation- only validation (DDE-R5); UNBUNDLE_SCAN/UNBUNDLE_EXECUTE two-phase protocol; GPCL-FCL-BR1 bridge rule; CL constraint_origin and imposed-citation rule CL-R9-imposed; reconciliation manifest schema formalized; derives derivation_mode subtype annotation; CIT-R6 traceability citation rule. |
...
| 6.1     | 2026-03-27 | Consistency patch release: semantic gap classification and INV-7; AX-5 wording refinement; optional cross-node semantic consistency review under VERIFY; explicit conflict resolution protocol; deferred-fragment handling for UNBUNDLE_EXECUTE; lifecycle completeness invariant INV-8; CIT-R7 parent-version freshness rule; schema and metadata alignment to v6.1.                                                                                                                                                                                                            |
...
| 6.3     | 2026-03-28 | Issue-resolution release: explicit document_profile root contract; canonical active_tiers closure; status_transitions as the sole lifecycle authority; deterministic ARE contract hardening; normalized operation namespace with UNBUNDLE_EXECUTE as the commit-phase token; centralized rule-ID typing; and structurally closed Express Mode groups and express-profile requirements.                                                                                                                                                                                           |
