...
8. Inspect `.agent/schemas/index.md` and confirm it contains exactly 14 data rows (one per schema), no `.bak` references, and all version/primary-skill values match the live schema directories.
...
- **Risk:** New governance rules introduce enforcement requirements that lag behind existing tooling.
  **Mitigation:** All three new rules describe structural requirements already implicit in the existing tools, evals, and config surfaces. No new validation scripts or enforcement tooling is introduced — the rules formalize what is already expected.
