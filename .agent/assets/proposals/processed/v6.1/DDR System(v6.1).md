...
### 1.1 Changes from v6.0

| Area                          | v6.0                                                                   | v6.1 (This Spec)                                                                                                                                   | Rationale                                                                                                                                      |
| ----------------------------- | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| Semantic gap governance       | `MISSING_MEDIATOR` existed as a manifest item type only                | `semantic_gap_classification` formalized; new `INV-7` requires explicit rationale, logging, and disposition before `CLEAN`                         | Makes tolerated structural-semantic gaps first-class, auditable, and bounded                                                                   |
| Constraint conflict handling  | Physical or imposed conflicts were described narratively               | `constraint_classes` and `physical_constraint_rule` formally distinguish logical precedence from non-overridable imposed or physical constraints   | Removes ambiguity around when escalation is mandatory and silent override is forbidden                                                         |
| Dirty state semantics         | Scoped SUPERSEDE DIRTY behavior existed only as narrative              | `dirty_classification` and `supersede_dirty_behavior` formalize structural versus semantic DIRTY handling                                          | Clarifies why rewiring can stay localized until semantic drift is actually detected                                                            |
| Cross-node semantic review    | VERIFY was defined as structural graph validation only                 | VERIFY may optionally evaluate `semantic_consistency_rules` and emit non-blocking `REVIEW_REQUIRED` items                                          | Enables graph-level semantic review without introducing a new Core operation                                                                   |
| Express Mode UNBUNDLE         | Any fragment not classified `high` blocked atomic UNBUNDLE             | Explicit `[DEFER]` annotation allows deferred-fragment handling while preserving atomicity for the executable subset                               | Keeps UNBUNDLE deterministic while reducing authoring brittleness                                                                              |
| Extension interaction wording | AX-5 implied Core should simply remain stable under Extensions         | AX-5 now clarifies that Extensions may interact with Core through explicitly defined, non-mutating interfaces                                      | Aligns the axiom wording with the actual extension model already present in the spec                                                           |
| Conflict resolution workflow  | Resolution workflow existed without a minimum conflict protocol        | New `conflict_resolution_protocol` formalizes identification, classification, escalation, recording, and remediation                               | Makes governance actions reproducible and auditable                                                                                            |
| Lifecycle completeness        | Lifecycle authority existed without an explicit completeness invariant | New `INV-8` requires a complete, closed lifecycle state machine                                                                                    | Protects AX-3 determinism by making lifecycle completeness normative                                                                           |
| Version coherence             | Parent-version freshness relied on implicit DIRTY propagation only     | New `CIT-R7` requires child re-validation after parent content version changes                                                                     | Prevents active nodes from silently depending on stale parent content                                                                          |

### 1.2 Errata Log

No active errata entries are carried in the authoritative v6.1 YAML definition.
...
- **INV-7** — Structural validity may coexist with declared semantic gaps only when those gaps are explicitly recorded in the reconciliation manifest under an allowed `semantic_gap_classification` type, with human rationale and required resolution or waiver before `CLEAN` state.
- **INV-8** — `lifecycle.status_transitions` must form a complete and closed state machine: every non-terminal status must have at least one valid outbound transition, and no undefined transitions are permitted.

> **Invariant Note:** VERIFY treats any node found in `SUPERSEDE_PENDING` as a `SUPERSEDE_PENDING_DETECTED` manifest item with severity `BLOCKING`. In v6.1, semantic gaps and lifecycle completeness are also made explicit invariants via `INV-7` and `INV-8`.
...
> `lifecycle.status_transitions` block in `ddr_system_v6.1.yaml`.
...
> **Machine Authority:** `ddr_system_v6.1.yaml` → `lifecycle.status_transitions`
...
>
> **Two-Phase UNBUNDLE Protocol:** `UNBUNDLE_SCAN` is independently invokable as a read-only pre-flight check that classifies each content fragment with confidence `high`, `ambiguous`, or `none`. `UNBUNDLE_EXECUTE` (invoked as the `UNBUNDLE` operation) is the atomic commit phase and may proceed only when each fragment is either classified as `high` or explicitly deferred under the deferred-fragment rule.
>
> **Deferred Fragment Handling (v6.1):** A fragment classified as `ambiguous` or `none` may be explicitly marked with a `[DEFER]` annotation and a recorded human rationale in the reconciliation manifest. Deferred fragments are excluded from the current UNBUNDLE execution and remain in the source Express Mode group node. Any fragment that is neither `high` nor explicitly deferred forces atomic rejection.
...
| `physical` | Represents non-negotiable physical realities or externally imposed constraints that cannot be silently overridden |
...
| **VERIFY**        | Traverse DAG downward; validate citation chains, edge types, ID references, orphans, contamination, and any declared cross-node semantic consistency rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Returns `CLEAN` or `DIRTY` with itemized structural violations. When `semantic_consistency_rules` are defined, VERIFY may also emit non-blocking `REVIEW_REQUIRED` items for human disposition                                                                                                                                                                                                                                                                                                            |
...
| **UNBUNDLE**      | Expand Express Mode group into constituent Full Mode tiers (atomic commit phase). Internally enforces UNBUNDLE_SCAN as a precondition; proceeds only when every fragment is either classified `high` or explicitly deferred.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Content allocated to correct tiers; `parent_ids` auto-wired. If any undeferred fragment carries confidence `ambiguous` or `none`, UNBUNDLE rejects atomically. No structural mutations are applied to rejected or deferred fragments. The rejection payload is the complete UNBUNDLE_SCAN result: a list of per-fragment diagnostic objects each containing `fragment_id`, `content_preview`, `detected_annotation`, `confidence`, and `ambiguity_reason`.                                                |
...
>
> **Operation Count:** DDR v6.1 defines 8 core operations. `UNBUNDLE_SCAN` remains the independently invokable pre-flight operation, and `UNBUNDLE` remains the atomic commit-phase operation.
...
> - **Dirty Classification (v6.1):** `DIRTY` now carries two normative semantic classes: `structural` for localized graph rewiring that does not itself prove semantic invalidation, and `semantic` for content drift that requires downstream review before `CLEAN` can be restored.
> - **Supersede DIRTY Behavior (v6.1):** Child nodes affected by parent rewiring enter `DIRTY` with classification `structural`. Structural DIRTY does not automatically propagate to descendants. If later VALIDATE or MODIFY activity reveals semantic drift, the affected node's DIRTY condition is upgraded to `semantic`, and ordinary downstream propagation rules then apply.
...
**Optional Semantic Consistency Rules:** VERIFY may evaluate cross-node semantic consistency rules within the same tier or across adjacent tiers. These findings are non-blocking structural advisories and must be emitted as `REVIEW_REQUIRED` items for human disposition.

### 7.4 Reconciliation Manifest Schema

The reconciliation manifest formally defines the following typed item types in v6.1:
...
- Allowed type: `MISSING_MEDIATOR`
- Constraint: must be explicitly logged in the reconciliation manifest
- Constraint: must carry human rationale
- Constraint: must be resolved or explicitly waived before system-wide `CLEAN`
...
**Contract:** ARE-1.0 / DDR-Core-6.x · **Reads:** ISL, CDL, ICL, SAL · **Annotates:** SAL, ICL, CDL, ISL · **Scoring Profile:** `standard_v1` (default)
...
| **Dirty Flag**         | `DIRTY` status indicating a node requires re-validation following a graph-modifying event. In v6.1, `DIRTY` may be classified as `structural` or `semantic` depending on whether semantic invalidation has been established                                                                                                                                                |
...
| **Semantic Gap**       | A declared condition where the graph remains structurally valid but semantically incomplete in a controlled, auditable way. In v6.1, only allowed `semantic_gap_classification` types may be used, and each must be resolved or explicitly waived before `CLEAN`                                                                                                           |
...
| 6.1     | 2026-03-27 | Consistency patch release: semantic gap classification and `INV-7`; AX-5 wording refinement; optional cross-node semantic consistency review under VERIFY; deferred-fragment handling for UNBUNDLE; explicit conflict resolution protocol; lifecycle completeness invariant `INV-8`; `CIT-R7` parent-version freshness rule; companion schema and metadata alignment to v6.1.                                                                                                                                                                                                      |
...
| v3.1.1 Tier | Current Destination   | Migration Notes                                                                                      |
| ----------- | --------------------- | ---------------------------------------------------------------------------------------------------- |
| XPD         | XPD                   | Unchanged                                                                                            |
| SIL         | SIL                   | Unchanged                                                                                            |
...
| v3.1.1 Rule ID         | Current Destination   | Consolidation Status | Notes                                                                                        |
| ---------------------- | --------------------- | -------------------- | -------------------------------------------------------------------------------------------- |
...
*DDR System v6.1 — Deterministic Design for Software Excellence*
