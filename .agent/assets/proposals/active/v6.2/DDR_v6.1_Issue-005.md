...
Keep the current namespaced annotation design, but add a second key-validation layer that rejects keys whose segment after `::` is `content`, `parent_ids`, `status`, `tier`, or `id`. In Draft 2020-12 the most precise shape is to validate `propertyNames` with an `allOf`: one rule keeps the positive namespace format, and a second `not` rule blocks the reserved suffixes. This preserves the current extension architecture while finally making the published safety constraint testable, and it avoids depending on regex lookahead features that are not part of the portable ECMA-262 subset JSON Schema relies on. It also avoids pushing collision-handling complexity into every downstream consumer.

* **Supporting Insights:** The schema already validates annotation key format, so the missing piece is not a new concept but a narrower key-name constraint. Tightening the schema here with `propertyNames` plus `not` keeps the Core/Extension boundary enforced at the intake point instead of relying on runtime conventions or on validator-specific regex behavior.
