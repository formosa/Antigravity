...
| :--- | :--- | :--- |
| **BRD** | "Graceful degradation under failure conditions" | Business resilience goal |
| **NFR** | "99.9% uptime, recovery within 5 seconds" | Quantified reliability |
| **FSD** | "System detects service faults and auto-recovers" | Observable behavior |
| **SAD** | "Supervisor pattern with heartbeat monitoring" | Architectural pattern |
| **ICD** | `{"command": "HEARTBEAT", "interval_ms": 1000}` | Message contract |
| **TDD** | "Class: FaultMonitor, Method: check_health()" | Component design |
...
-   **No Business Context:** Why does 100�s matter to users/business?
...
| Sibling citations | "FSD-4.2 cites FSD-4.1" | Cite common parent instead, not sibling tags |
| Renumbering tags | Changing `\|FSD-4\|` to `\|FSD-5\|` | Never renumber; deprecate and create new |
| No rationale | Design choice without justification | Add `RATIONALE: ...` in parent tag |
...
tools:
  ...
  - name: deprecate_tag
    description: Mark tag as deprecated without deletion
    input_schema:
      type: object
      properties:
        tag_id:
        ...
        replacement_tag:
          type: string
          description: Optional ID of replacement tag
        deprecation_reason:
          type: string
        version:
          type: string
      required: [tag_id, version]

    implementation: |
      def deprecate_tag(tag_id, replacement_tag=None, deprecation_reason=None, version=None):
  ...
  - name: extract_citations
    description: Parse parent citations from tag content
    input_schema:
      type: object
      properties:
        content:
    ...
    output_schema:
      ...
      items:
        type: string
      description: List of cited parent tag IDs

    implementation: |
      import re

      def extract_citations(content):
          # Pattern: ? |TAG-ID| or ? |TAG-1|, |TAG-2|
          pattern = r'?\s*\|([A-Z]{3}-\d+(?:\.\d+)?)\|'
          matches = re.findall(pattern, content)
          return matches

  - name: find_tags_citing
    description: Find all tags that cite a given parent tag
    input_schema:
      type: object
      properties:
        parent_tag:
    ...
    implementation: |
      def find_tags_citing(parent_tag):
          results = []
...
Orphan_Detective: ?? Orphan tag detected: |FSD-21.3|
...
BRD_Strategist: ?? Tier Violation Detected
...
4. ?? Learn more about BRD tier rules
