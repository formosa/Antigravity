"""
Lightweight Traceability Audit for Sphinx-Needs JSON.

This script performs structural validation on the Sphinx-Needs JSON export
(`needs.json`) to detect orphaned items, broken links, and hierarchy
violations. It operates exclusively on the JSON representation without
parsing RST source files.

Purpose
-------
Validate the logical structure of documentation traceability:

- **Orphan Detection**: Items with no parent links (except BRD root)
- **Broken Links**: References to non-existent item IDs
- **Lateral Dependencies**: Same-level sibling links (violations)
...
Command-line execution::
    ...
    python audit_traceability.py [needs_json_path]

Default path (if not specified)::

    python audit_traceability.py
    # Uses: docs/_build/json/needs.json

Example with explicit path::

    python audit_traceability.py docs/_build/json/needs.json

Hierarchy Rules
---------------
The script validates upward traceability (child → parent):

=========== ===== ===============================
Level       Order Expected Parent Levels
=========== ===== ===============================
ISP (test)    0   TDD, ICD, SAD, FSD, NFR, BRD
TDD (impl)    1   ICD, SAD, FSD, NFR, BRD
ICD (schema)  2   SAD, FSD, NFR, BRD
SAD (arch)    3   FSD, NFR, BRD
FSD (spec)    4   NFR, BRD
NFR (const)   5   BRD
BRD (req)     6   None (root level)
=========== ===== ===============================
...
::
    ...
    Auditing docs/_build/json/needs.json...
    Versions: ['0.1']
    ...
    --- Issues ---
    Orphan: FSD-12 has no parents.
    Lateral Dependency: TDD-3.1 links to sibling TDD-3.2

See Also
--------
- Root `audit_traceability.py` : Full-featured version with RST scanning
...
def load_needs_json(path):
    """
    Load and parse the Sphinx-Needs JSON export file.
    ...
    -------
    dict
        Dictionary of needs items keyed by item ID.
