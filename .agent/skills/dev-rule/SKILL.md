...
2. Decide whether to scaffold or refine:
   - Run `python .agent/skills/dev-rule/scripts/init_rule.py <rule-name> [--path <output-directory>]` when creating a new rule.
...
5. Validate with `python .agent/skills/dev-rule/scripts/quick_validate.py <rule-path-or-directory>`. Resolve structural failures before proceeding.
6. Update the rules directory index with `python .agent/skills/dev-rule/scripts/update_index.py` so `.agent/rules/index.md` stays aligned with the live rule set.
...
<resources_reference>
- Run `.agent/skills/dev-rule/scripts/init_rule.py` to scaffold a new rule from the canonical rule example.
- Run `.agent/skills/dev-rule/scripts/quick_validate.py` to detect rule-asset structural errors before handoff.
- Run `.agent/skills/dev-rule/scripts/update_index.py` to rebuild `.agent/rules/index.md` after rule changes.
- Read `.agent/skills/dev-skill/resources/owner-skill-pattern.md` to preserve the shared owner-skill lifecycle and governance model while keeping rule responsibilities role-appropriate.
