...
7. Sync vendored schema mirrors with `python .agent/skills/dev-skill/scripts/sync_schema_mirrors.py <path-to-skill>` so `resources/schema/<schema-id>/` reflects the canonical `.agent/schemas/` directories before validation or packaging.
8. Validate with `python .agent/skills/dev-skill/scripts/quick_validate.py <path-to-skill>`. Resolve all structural errors, then address any quality warnings about weak descriptions, missing exclusions, or ambiguous resource entries.
...
10. Package with `python .agent/skills/dev-skill/scripts/package_skill.py <path-to-skill> [output-directory]` only after schema sync, validation, and trigger checks are clean enough for handoff.
</how_to_use>
...
<resources_reference>
- Run `.agent/skills/dev-skill/scripts/init_skill.py` to scaffold the minimal skill directory and optional folders.
...
- Read `.agent/skills/dev-skill/resources/schema/skill/README.md` to understand the canonical-versus-vendored schema governance model.
- Read `.agent/skills/dev-skill/directory_structure.md` to choose the smallest correct scaffold shape before adding optional folders.
</resources_reference>
