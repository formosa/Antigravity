<document_purpose>
This template defines the canonical `v6.1` Resolution Report structure used by
`agent-create-issue-report`. New reports must follow this shape exactly. Legacy `v4` and
`v5` reports remain valid historical artifacts, but they are not the generation target.
</document_purpose>
