...
- Legacy requests that still say `dev-schema` should route here; `core-schema` is the active foundational schema-authoring contract.
...
8. **IF** validation passes, **THEN** update the schema directory index:
   - `python .agent/skills/core-schema/scripts/update_index.py`
...
- Read `.agent/skills/core-schema/config.json` to check the resolved schema output directory and auto-validation toggle.
</resources_reference>
