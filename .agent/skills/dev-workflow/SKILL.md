...
2. Decide whether to scaffold or refine:
   - Run `python .agent/skills/dev-workflow/scripts/init_workflow.py <workflow-name> [--path <output-directory>]` when creating a new workflow.
...
5. Validate with `python .agent/skills/dev-workflow/scripts/quick_validate.py <workflow-path>`. Resolve structural failures before proceeding.
6. Update the workflow directory index with `python .agent/skills/dev-workflow/scripts/update_index.py` so `.agent/workflows/index.md` stays aligned with the live workflow set.
...
- Run `.agent/skills/dev-workflow/scripts/init_workflow.py` to scaffold a new workflow from the canonical example structure.
- Run `.agent/skills/dev-workflow/scripts/quick_validate.py` to detect structural workflow errors before handoff.
- Run `.agent/skills/dev-workflow/scripts/update_index.py` to rebuild `.agent/workflows/index.md` after workflow changes.
- Read `.agent/skills/dev-skill/resources/owner-skill-pattern.md` to preserve the shared owner-skill lifecycle and governance model while keeping workflow responsibilities role-appropriate.
