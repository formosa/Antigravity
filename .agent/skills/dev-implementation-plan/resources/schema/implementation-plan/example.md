...
3. Inspect `src/core/worker.py`: confirm `QRunnable` subclass, `WorkerSignals(QObject)` definition, and at least one typed signal (for example `result = Signal(object)`).
