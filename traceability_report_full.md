...
| `ICD-1` | IPC Configuration (ipc_config.yaml) | **ERROR** | NFR-3.6 (**Memory Footprint (Audio Queue):** 50 cap, ~1-5 MB.) | Invalid parent tier 'NFR' for 'ICD'. Allowed: ['SAD'] |
| `ICD-1` | IPC Configuration (ipc_config.yaml) | **ERROR** | NFR-3 (Resource Isolation) | Invalid parent tier 'NFR' for 'ICD'. Allowed: ['SAD'] |
| `ICD-3` | Metadata Schema (JSON) | **ERROR** | FSD-6.3 (**Correlation:** All logs must include `request_id` for correlation across processes.) | Invalid parent tier 'FSD' for 'ICD'. Allowed: ['SAD'] |
